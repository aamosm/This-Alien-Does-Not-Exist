...
function isNonEmptyVector(v) {
    ...
    if (ArrayBuffer.isView(v)) return v.length > 0; // typed arrays
    return false;
}
...
function hashVector(vec) {
    ...
    for (let i = 0; i < vec.length; i++) {
        // Quantize to keep the hash stable for near-identical floats
...
function behaviourKeysFor(registry) {
    return Object.keys(registry || {}).filter(k => k !== 'physicalSignature');
}
...
function computeBehaviourDistance(a, b) {
    const aKeys = a && a.keys, bKeys = b && b.keys;
    const va = a && a.vector ? a.vector : toArray(a);
    const vb = b && b.vector ? b.vector : toArray(b);
    if (!isNonEmptyVector(va) || !isNonEmptyVector(vb) || va.length !== vb.length) {
        warn('computeBehaviourDistance: vectors missing or length mismatch (' +
            (va ? va.length : 0) + ' vs ' + (vb ? vb.length : 0) + ').');
        return Infinity;
    }
    const keys = aKeys || bKeys || va.map((_, i) => `dim${i}`);
    ...
    const weights = na.map(() => 1); // behavioural dims are unweighted relative to each other
...
function computeSimilarity(a, b) {
    ...
    const ba = a.behaviourVector, bb = b.behaviourVector;
    const behaviourDist = computeBehaviourDistance(ba, bb);
    const bva = ba && ba.vector ? ba.vector : toArray(ba);
    const bvb = bb && bb.vector ? bb.vector : toArray(bb);
    const behaviourEuclideanSim = squash(behaviourDist);
    const behaviourCosineSim = clamp((cosineSimilarity(bva, bvb) + 1) / 2, 0, 1);
...
function dimensionStats(database) {
    if (!isValidDatabase(database) || database.length === 0) return [];

    const validEntries = database.filter(e => e && isNonEmptyVector(e.physicalVector));
...
function computeUniqueTraits(alien, database) {
    ...
    const stats = dimensionStats(database);
...
function computeCluster(seed, database) {
    ...
    let h = 0x9e3779b9;
    vec.forEach((v, i) => {
        const bucket = clamp(Math.floor(v * CLUSTER_BUCKETS_PER_DIM), 0, CLUSTER_BUCKETS_PER_DIM - 1);
